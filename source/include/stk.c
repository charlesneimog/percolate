...
{                                                     /*  sample is differential  */
	...
	if (bowtable->lastOutput > 1.0)
		bowtable->lastOutput = 1.0; /*  maximum friction is 1.0 */
...
float ReedTabl_lookup(ReedTabl* reedtable, float deltaP) {
	...
	if (reedtable->lastOutput > 1.0)
		reedtable->lastOutput = 1.0; /* if other way, reed slams shut   */
	if (reedtable->lastOutput < -1.0)
		reedtable->lastOutput = -1.0; /* if all the way open, acts like open end */
...
void LipFilt_setFreq(LipFilt* lipfilt, float frequency, float srate) {
	...
	if (srate < 22050.)
		srate = 22050.;
	coeffs[0] = 2.0 * 0.997 * cos(TWO_PI * frequency / srate); /* damping should change with  */
...
void DLineA_alloc(DLineA* delayLine, long max_length) {
	...
	delayLine->inputs = getbytes(delayLine->length * sizeof(double));
	if (!delayLine->inputs) {
		perror("DlineA: out of memory");
		return;
	}
...
void DLineA_setDelay(DLineA* delayLine, double lag) {
	...
	delayLine->coeff = (1.0 - delayLine->alpha) / (1.0 + delayLine->alpha);    // coefficient for all pass
}
...
float DLineL_tick(DLineL* delayLine, float sample)    // Take one, yield one
{
	...
	delayLine->lastOutput = delayLine->inputs[delayLine->outPoint++] * delayLine->omAlpha;     // first 1/2 of interpolation
	if (delayLine->outPoint < delayLine->length) {                                             // Check for end condition
		delayLine->lastOutput += delayLine->inputs[delayLine->outPoint] * delayLine->alpha;    // second 1/2 of interpolation
	}
	else {                                                                   // if at end . . .
		delayLine->lastOutput += delayLine->inputs[0] * delayLine->alpha;    // second 1/2 of interpolation
		delayLine->outPoint -= delayLine->length;
	}
...
void DLineN_setDelay(DLineN* delayLine, float lag) {
	...
	while (delayLine->outPoint < 0)
		delayLine->outPoint += delayLine->length;    // modulo maximum length
}
...
void RawWvIn_setRate(RawWvIn* inwave, float aRate) {
	inwave->rate = aRate;
	if (fmod(inwave->rate, 1.0) > 0.0)
		inwave->interpolate = 1;
	else
		inwave->interpolate = 0;
}
...
int RawWvIn_informTick(RawWvIn* inwave) {
	...
	if (!inwave->finished) {
		...
		if (inwave->interpolate) {
			...
			inwave->lastOutput = inwave->lastOutput + (alpha * (inwave->data[temp + 1] - inwave->lastOutput));
		}
		else {
			inwave->lastOutput = inwave->data[temp];
		}
...
void Modal4_setRatioAndReson(Modal4* modal, int whichOne, float ratio, float reson) {
	...
	if (ratio * modal->baseFreq < modal->srate * 0.5) {
		...
		temp = ratio;
		while (temp * modal->baseFreq > modal->srate * 0.5)
			temp *= 0.5;
...
float Modal4_tick(Modal4* modal) {
	...
	temp  = modal->masterGain * OnePole_tick(&modal->onepole, Envelope_tick(&modal->envelope) * modal->wave.lastOutput);
